...
def Face(df, count):
    ...
    alignstr = '%dx%d+%d+%d' % (width, height, (screenwidth - width) /3, (screenheight - height) / 5)
    ...
    def exit():
        df.to_excel('词库.xlsx', index=None)
        root.destroy()
        sys.exit()
    ...
    while True:
        ...
        if df.at[num, '是否认识'] == '否':
            break
    label_2 = tkinter.Label(root, text="词库总数： " + str(df.shape[0]) + '已认识数：' + str(df2.shape[0]), width=30, height=2, bg='#6894B9')
    label_2.pack()
    ...
    button4 = tkinter.Button(root, bg='#c4c3be', text='被查阅次数:  ' + str(df.at[num, '查阅次数']), font=('黑体', 15), width=40, height=2)
    button4.pack()

    button2 = tkinter.Button(root, bg='#f3c5b3', text=df.at[num, '单词'],command=select_chiness,  font=('黑体', 15), width=40, height=5)
    button2.pack()
